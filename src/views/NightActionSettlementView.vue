...
async function fetchActions() {
  ...
  try {
    ...
    const result = await nightActionAPI.getAllActions(params)
...
async function submitFeedback() {
  ...
  try {
    const res = await nightActionAPI.feedbackAction(selectedActionId.value, feedbackText.value.trim())
    if (res?.success) {
      feedbackText.value = ''
      selectedActionId.value = null
      showToast('success', '结算结果已保存')
      await fetchActions()
    } else {
...
<template>
  <div>
      ...
      <div class="bg-gradient-to-br from-[#1a2332] to-[#0f1419] border border-white/10 rounded-2xl p-5 mb-6">
        ...
        <p class="text-xs text-gray-500 mt-3">待结算：<span class="text-amber-400 font-medium">{{ pendingCount }}</span></p>
      </div>
      ...
      <div v-else-if="filteredActions.length === 0" class="text-center py-20 text-gray-500">暂无夜晚行动记录</div>
      ...
      <div v-else class="space-y-4">
        <div
          ...
          :class="{ 'ring-1 ring-indigo-500/30': selectedActionId === action.id }"
        >
          <div class="flex flex-wrap items-center gap-2 mb-3">
            <span class="text-white font-medium text-sm">{{ action.playerName }}</span>
            <span
              class="text-xs px-2 py-0.5 rounded-full border"
              :class="FACTION_LABELS[action.faction]?.color || 'bg-white/10 text-gray-400'"
            >
              {{ FACTION_LABELS[action.faction]?.label || action.faction }}
            </span>
            <span class="text-xs px-2 py-0.5 rounded-full bg-indigo-500/20 text-indigo-300">{{ action.actionTypeLabel }}</span>
            ...
            >
              {{ action.status === 'pending' ? '待结算' : '已结算' }}
            </span>
          </div>

          <pre
            v-if="action.payload && Object.keys(action.payload).length"
            class="text-gray-500 text-xs mb-2 whitespace-pre-wrap font-sans"
          >提交详情：
{{ formatPayload(action.payload) }}</pre>

          <div
            v-if="action.result"
            class="text-gray-300 text-sm whitespace-pre-wrap bg-black/20 rounded-xl p-4 mb-3 border border-white/5"
          >
            {{ action.result }}
          </div>
          ...
          <div v-if="action.status === 'pending'" class="flex gap-2">
            <button
              type="button"
              class="px-4 py-1.5 bg-indigo-600/20 text-indigo-300 border border-indigo-500/30 rounded-lg text-sm hover:bg-indigo-600/30"
              @click="selectedActionId = action.id; feedbackText = ''"
            >
              {{ selectedActionId === action.id ? '填写结算中...' : '填写结算' }}
            </button>
          </div>
          ...
          <div v-if="selectedActionId === action.id" class="mt-3 pt-3 border-t border-white/10">
            <textarea
              ...
              rows="4"
              placeholder="填写结算结果：成功率、防御值对比、成败、暴露、伤亡等..."
              class="w-full resize-none bg-black/30 border border-white/10 rounded-xl px-4 py-3 text-sm text-gray-200 mb-2 focus:outline-none focus:border-indigo-500/50"
            />
            <div class="flex gap-2">
              <button type="button" class="px-4 py-1.5 bg-indigo-600 text-white rounded-lg text-sm" @click="submitFeedback">
                保存结算
              </button>
              <button
                type="button"
                class="px-4 py-1.5 bg-white/5 text-gray-400 rounded-lg text-sm"
                @click="selectedActionId = null; feedbackText = ''"
              >
                取消
              </button>
            </div>
